-- Run this ONLY if you already ran 001 (or setup_all failed partway).
...
DROP POLICY IF EXISTS "Owner can delete empty family" ON public.families;
...
DROP POLICY IF EXISTS "Family can view credentials" ON public.shared_credentials;
DROP POLICY IF EXISTS "Family can add credentials" ON public.shared_credentials;
DROP POLICY IF EXISTS "Family can update credentials" ON public.shared_credentials;
DROP POLICY IF EXISTS "Family can delete credentials" ON public.shared_credentials;

DROP POLICY IF EXISTS "Family can view household" ON public.household_items;
DROP POLICY IF EXISTS "Family can add household" ON public.household_items;
DROP POLICY IF EXISTS "Family can update household" ON public.household_items;
DROP POLICY IF EXISTS "Family can delete household" ON public.household_items;
...
CREATE INDEX IF NOT EXISTS idx_shared_credentials_family ON public.shared_credentials(family_id);
CREATE INDEX IF NOT EXISTS idx_household_items_family ON public.household_items(family_id);
CREATE INDEX IF NOT EXISTS idx_families_invite_code ON public.families(invite_code);

-- Auto-create family on signup (replaces basic profile-only trigger)
...
DROP TRIGGER IF EXISTS update_shared_credentials_updated_at ON public.shared_credentials;
